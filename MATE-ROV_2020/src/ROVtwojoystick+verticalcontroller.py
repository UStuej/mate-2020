...
def sendControls(controls):
    try:
        sock.sendto(("apply_controls(%s);" % str(controls)).encode(), ADDRESS)
    except BaseException as error:
...
while running:
    ...
    #print ("rightJoypos:", rightJoyYPos)
        
    #print ( "joystickPos = ", round(rightJoyXPos, 2), round(rightJoyYPos, 2))
      # to prevent a divide by zero error, we put the joystick angle at zero
    if (rightJoyYPos == 0):
        if (rightJoyXPos >= 0):
...
##    print ("joystick angle = ", joystickAngle)
##    print ("joystick angle DEG = ", joystickAngle*180.0/math.pi)
    #joystickAngle = math.atan(rightJoyXPos/rightJoyYPos)
...
##       
    #print("Motor X,Y = ", round(rawmotorX,2), round(rawmotorY,2))   
    #print("Motor X,Y = ", round(rawmotorX,2), round(rawmotorY,2), round(motorAngle*180.0/math.pi,2))
    try:
    ...
    #print ("scale = ", scale)
         ...
         ##SCALING THE X MOTORS TO THREE SPEED ZONES
...
##        
    ...
    #print("scaledrightX, scaledrightY = ", round(scaledmotorX,2), round(scaledmotorY,2))

    

              # CONTROLLING TWO MOTORS WITH THE LEFT JOYSTICK#

    leftJoyYPos = -joystick.get_axis(leftJoyYAxisIndex)
    leftJoyXPos = joystick.get_axis(leftJoyXAxisIndex)
    #print("leftY, leftX, rightY, rightX = ", round(leftJoyYPos,2), round(leftJoyXPos,2), round(rightJoyYPos,2), round(rightJoyXPos,2))
##
##
    if (leftJoyYPos == 0):
       if (leftJoyXPos >= 0):
    ...
    #print(round(debug_vpower,2), round(joystick.get_axis(0),2), round(joystick.get_axis(1),2), round(joystick.get_axis(2),2), round(joystick.get_axis(3),2), round(joystick.get_axis(4),2))
    
    #print("upmotor, downmotor = ", round(upmotor,2), round(downmotor,2))
    #print("scaledleftX, scaledleftY = ", round(scaledmotorX,2), round(scaledmotorY,2))
    ...
    pygame.draw.lines(screen, RGB_BLUE, False, [(MY1_X,MY1_Y), (MY1_X-scaledrightmotorY*50,MY1_Y-scaledrightmotorY*50)], 2)
        ...
        ##pygame.draw.lines(screen, RGB_RED, False, [(MX1_X,MX1_Y), (MX1_X+rawmotorX*50,MX1_Y-rawmotorX*50)], 2)
        ##pygame.draw.lines(screen, RGB_RED, False, [(MX2_X,MX2_Y), (MX2_X+rawmotorX*50,MX2_Y-rawmotorX*50)], 2)
         
        # pygame.draw.polygon(screen, RGB_RED, [(MX1_X-20,MX1_Y-20), (MX1_X+rawmotorX*50,MX1_Y-rawmotorX*50), (MX1_X+20,MX1_Y+20)])


    ##    drawArrow (screen, RGB_RED, MX1_X, MX1_Y, rawmotorX, "X")
    ##    drawArrow (screen, RGB_RED, MX2_X, MX2_Y, rawmotorX, "X")
    ##    drawArrow (screen, RGB_BLUE, MY1_X, MY1_Y, rawmotorY, "Y")
    ##    drawArrow (screen, RGB_BLUE, MY2_X, MY2_Y, rawmotorY, "Y")

    pygame.draw.lines(screen, RGB_RED, False, [(MX1_X,MX1_Y), (MX1_X+scaledleftmotorX*50,MX1_Y-scaledleftmotorX*50)], 2)

    ##    if rawmotorX > 0 :
    ##        pygame.draw.polygon(screen, RGB_RED, [(MX1_X+rawmotorX*50-8, MX1_Y-rawmotorX*50-8),
    ##                                              (MX1_X+rawmotorX*50+5, MX1_Y-rawmotorX*50-5),
    ##                                              (MX1_X+rawmotorX*50+8, MX1_Y-rawmotorX*50+8)])
    ##
    ##
    ##    elif rawmotorX < 0 :
    ##        pygame.draw.polygon(screen, RGB_RED, [(MX1_X+rawmotorX*50-8, MX1_Y-rawmotorX*50-8),
    ...
    ##    if rawmotorX > 0 :
    ##        pygame.draw.polygon(screen, RGB_RED, [(MX2_X+rawmotorX*50-8, MX2_Y-rawmotorX*50-8),
    ##                                              (MX2_X+rawmotorX*50+5, MX2_Y-rawmotorX*50-5),
    ##                                              (MX2_X+rawmotorX*50+8, MX2_Y-rawmotorX*50+8)])
    ##
    ##
    ##    elif rawmotorX < 0 :
    ##        pygame.draw.polygon(screen, RGB_RED, [(MX2_X+rawmotorX*50-8, MX2_Y-rawmotorX*50-8),
    ...
    sendControls({"top_left":scaledleftmotorX, "top_right":scaledrightmotorY, "bottom_left":scaledleftmotorY, "bottom_right":scaledrightmotorX, "vertical_left":vmotorpower, "vertical_right":vmotorpower})

       # pygame.draw.lines(screen, RGB_BLUE, False, [(MY1_X,MY1_Y), (MY1_X-rawmotorY*50,MY1_Y-rawmotorY*50)], 2)
       #pygame.draw.lines(screen, RGB_BLUE, False, [(MY1_X,MY1_Y), (MY1_X-rawmotorY*50,MY1_Y-rawmotorY*50)], 2)

    ##    if rawmotorY > 0 :
    ##        pygame.draw.polygon(screen, RGB_BLUE, [(MY1_X-rawmotorY*50-5, MY1_Y-rawmotorY*50-5),
    ##                                               (MY1_X-rawmotorY*50+8, MY1_Y-rawmotorY*50-8),
    ##                                               (MY1_X-rawmotorY*50+5, MY1_Y-rawmotorY*50+5)])
    ##
    ##
    ##    elif rawmotorY < 0 :
    ##        pygame.draw.polygon(screen, RGB_BLUE, [(MY1_X+rawmotorY*50-5, MY1_Y+rawmotorY*50-5),
    ...
    ##    pygame.draw.lines(screen, RGB_BLUE, False, [(MY2_X,MY2_Y), (MY2_X-rawmotorY*50,MY2_Y-rawmotorY*50)], 2)
    ##
    ##    if rawmotorY > 0 :
    ##        pygame.draw.polygon(screen, RGB_BLUE, [(MY2_X+rawmotorY*50-8, MY2_Y-rawmotorY*50-8),
    ##                                               (MY2_X+rawmotorY*50+5, MY2_Y-rawmotorY*50-5),
    ##                                               (MY2_X+rawmotorY*50+8, MY2_Y-rawmotorY*50+8)])
    ##
    ##
    ##    elif rawmotorY < 0 :
    ##        pygame.draw.polygon(screen, RGB_BLUE, [(MY2_X+rawmotorY*50-8, MY2_Y-rawmotorY*50-8),
