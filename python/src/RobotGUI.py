...
class GPIOMotor(AbstractMotor):
    ...
    def update(self):
        GPIO_BACKEND_PIN_SET_POWER(self.pin,
                                   self.speed)  # Actual range of PWM control output must be from 1000-2000. Use translate in backend function to accomodate this
...
class AbstractServo(object):
    ...
    def __repr__(self):
        return type(self).__name__ + "(initial_angle=%s, min_angle=%s, max_angle=%s)" % (
            self.initial_angle, self.min_angle, self.max_angle)
...
class TankSteeringController(object):
    ...
    def __repr__(self):
        return type(self).__name__ + "(%s, %s, %s, %s)" % (
...
class HexROVController():
    ...
    def __init__(self, front_left, front_right, back_left, back_right,
                 deadzone=0):  # TODO: Copy Elis' code from twoJoystickController program to implement proper tank steering with the ROV
        self.front_left = front_left
        self.front_right = front_right
        self.back_left = back_left
        self.back_right = back_right
    ...
    def __repr__(self):
        return type(self).__name__ + "(%s, %s, %s, %s, deadzone=%s)" % (
            self.front_left, self.front_right, self.back_left, self.back_right, self.deadzone)
...
class TargetNavigator(object):
    ...
    def __repr__(self):
        return type(self).__name__ + "(%s, %s, target=%s, power_function=%s)" % (
            self.controller, self.screen_size, self.target, self.power_function)
...
class LineNavigator(object):
    ...
    def __repr__(self):
        return type(self).__name__ + "(%s, %s, %s, direction=%s)" % (
            self.navigator, self.tracker, self.screen_size, self.direction)
...
def assemble_controller():
    ...
    if controller_name == "Hex ROV Controller":
        controller = HexROVController(*[motors.get(pin, AbstractMotor()) for pin in hex_rov_pins.values()],
                                      deadzone=deadzone)
    elif controller_name == "Tank Steering Controller":
        controller = TankSteeringController(*[motors.get(pin, AbstractMotor()) for pin in tank_rov_pins.values()],
                                            deadzone=deadzone)
    elif controller_name == "Object Navigator":
        if type(controller) == HexROVController:
            controller = ObjectNavigator(TargetNavigator(controller, size),
                                         ObjectTracker(camera.read(), (0, 0, 0, 0), 1),
                                         camera)  # TODO: User input initial bounding box and accuracy
        else:
            controller = ObjectNavigator(TargetNavigator(controller.navigator.controller),
                                         ObjectTracker(camera.read(), (0, 0, 0, 0), 1), camera)
...
if __name__ == "__main__" and USE_GUI:
    ...
    hex_rov_pins = {"Front Left Motor": None, "Front Right Motor": None, "Back Left Motor": None,
                    "Back Right Motor": None}
    tank_rov_pins = hex_rov_pins.copy()

    controller_input_names = {"Hex ROV Controller": ["X", "Y"], "Tank Steering Controller": ["Left", "Right"],
                              "Target Navigator": [], "Object Navigator": [], "Line Navigator": []}
    ...
    while True:
        ...
        if imgui.begin_main_menu_bar():
            if imgui.begin_menu("File"):
                ...
                if clicked:
                    try:
                        ...
                        for item in ["local_motors", "other", "available_pins", "hex_rov_pins", "tank_rov_pins",
                                     "deadzone", "rov_type", "pin_values", "controller_inputs", "controller_name",
                                     "local_input", "local_camera"]:
            ...
            if imgui.begin_menu("Controller Type"):
                for controller_type in ["Hex ROV Controller", "Tank Steering Controller", "Target Navigator",
                                        "Object Navigator", "Line Navigator"]:
        ...
        for pin in available_pins:
            if pin[2]:
                changed, new_value = imgui.slider_float(
                    "GPIO #" + str(pin[0]) + ", BCM #" + str(pin[1]) + " (Hardware PWM)", motors[pin].speed, -1, 1)
            else:
                changed, new_value = imgui.slider_float("GPIO #" + str(pin[0]) + ", BCM #" + str(pin[1]),
                                                        motors[pin].speed, -1, 1)
        ...
        for motor in ["Front Left Motor", "Front Right Motor", "Back Left Motor", "Back Right Motor"]:
            ...
            if rov_pins[motor]:
                if rov_pins[motor][2]:
                    imgui.text(motor + ": GPIO #" + str(rov_pins[motor][0]) + ", BCM #" + str(
                        rov_pins[motor][1]) + " (Hardware PWM)")
                else:
